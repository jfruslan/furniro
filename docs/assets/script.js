...
function showMore() {
  ...
  const productsBodyItems = document.querySelectorAll(".products-body-item");
  if (button) {
    button.addEventListener("click", () => {
      const productsButtonText = document.querySelector(".products-button-text");
      if (!productsButtonText.classList.contains("_less")) {
        productsButtonText.remove();
        button.insertAdjacentHTML("beforeend", '<p class="products-button-text _less">Show Less</p>');
      } else {
        productsButtonText.remove();
        button.insertAdjacentHTML("beforeend", '<p class="products-button-text">Show More</p>');
      }
      productsBodyItems.forEach((productsBodyItem) => {
        if (!productsBodyItem.classList.contains("_more")) {
          productsBodyItem.classList.add("_more");
        } else {
          productsBodyItem.classList.remove("_more");
...
function productsActions() {
  document.addEventListener("click", (e) => {
    ...
    const productsBodyItem = targetElement.closest(".products-body-item");
    showActions(productsBodyItem);
    if (targetElement.classList.contains("products-body-item-actions-button")) {
      const productId = productsBodyItem.dataset.pid;
    ...
    if (targetElement.classList.contains("cart-header__icon") || targetElement.closest(".cart-header__icon")) {
      ...
      e.preventDefault();
    } else if (!targetElement.closest(".cart-header") && !targetElement.classList.contains("products-body-item-actions-button")) {
...
function showActions(productsBodyItem) {
  const actions = document.querySelector("._actions");
  actions && actions.classList.remove("_actions");
  if (productsBodyItem) {
    if (!productsBodyItem.classList.contains("_actions")) {
      productsBodyItem.classList.add("_actions");
    } else {
      productsBodyItem.classList.remove("_actions");
    }
  } else {
  }
}
function addToCart(productButton, productId) {
  if (!productButton.classList.contains("_hold")) {
    ...
    const productImage = product.querySelector(".products-body-item-wrapimg");
    ...
    const cartFlyLeft = cart.getBoundingClientRect().left + 15;
    const cartFlyTop = cart.getBoundingClientRect().top + 15;
    productImageFly.style.cssText = `
			left: ${cartFlyLeft}px;
			top: ${cartFlyTop}px;
			width: 0px;
...
function updateCart(productButton, productId, productAdd = true) {
  ...
  if (productAdd) {
    ...
    if (!cartProduct) {
      const product = document.querySelector(`[data-pid="${productId}"]`);
      const cartProductImage = product.querySelector(".products-body-item-wrapimg").innerHTML;
      const cartProductTitle = product.querySelector(".products-body-item-title").innerHTML;
      const cartProductContent = `
		<a href="#a" class="cart-list__image _ibg">${cartProductImage}</a>
		<div class="cart-list__body">
			<a href="#a" class="cart-list__title">${cartProductTitle}</a>
			<div class="cart-list__quantity">Quantity: <span>1</span></div>
			<a href="#a" class="cart-list__delete">Delete</a>
		</div>`;
...
function myGalleries() {
  ...
  if (galleries.length) {
    ...
    galleries.forEach((gallery) => {
      galleyItems.push({
        gallery,
        galleryClass: lightgallery_es5_default(gallery, {
          // plugins: [lgZoom, lgThumbnail],
          licenseKey: "7EC452A9-0CFD441C-BD984C7C-17C8456E",
          speed: 500,
          selector: ".furniture__row-item-wrapimg"
        })
      });
    });
...
function initSliders() {
  ...
  if (document.querySelector(".rooms__swiper")) {
  ...
  if (document.querySelector(".tips__slider")) {
    new Swiper(".tips__slider", {
      ...
      // Arrows
      navigation: {
        nextEl: ".swiper-button-next.tips__swiper-button-next",
        prevEl: ".swiper-button-prev.tips__swiper-button-prev"
      },
      breakpoints: {
        240: {
          slidesPerGroup: 1,
